...
task :build => [:clean] do
  ...
  # copy src
  cp 'jquery.lifesupport.js', 'dist/jquery.lifesupport.js'
  ...
  # minify src
  source = File.read('dist/jquery.lifesupport.js')
  minified = Packr.pack(source, :shrink_vars => true, :base62 => false)
  header = /\/\*.*?\*\//m.match(source)

  # inject header
  File.open('dist/jquery.lifesupport.min.js', 'w') do |combined|
...
task :release => [:build] do
  root = pwd+'/dist'
  zip_archive = pwd+'/dist/jquery.lifesupport.zip'
...
task :test => [:build] do  
  begin
    # mac
    sh("open spec/jquery.lifesupport.specs.html")
  rescue
    # windows
    sh("start spec/jquery.lifesupport.specs.html")
  end
end
